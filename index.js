...
const getDefaultPlugins = async () => {
  const plugins = await Promise.all(defaultPlugins.map(async plugin => {
    if (plugin === 'gifsicle') return loadPlugin('gifsicle', { interlaced: true, optimizationLevel: 2 });
    if (plugin === 'mozjpeg') return loadPlugin('mozjpeg', { quality: 90, progressive: true });
    if (plugin === 'optipng') return loadPlugin('optipng', { optimizationLevel: 3 }); // Safer level
    if (plugin === 'svgo') return loadPlugin('svgo', {
...
export default function gulpImagemin(plugins, options) {
  ...
  return gulpPlugin('gulp-imagemin', async file => {
    ...
    if (Array.isArray(plugins)) {
      localPlugins = await Promise.all(plugins);
    } else {
      localPlugins = await getDefaultPlugins();
    }

    // Filter out any null plugins (failed to load)
    localPlugins = localPlugins.filter(plugin => plugin !== null);
